...
async def _post_init(app):
    ...
    # Start awakening monitor (Tier B early-warning system)
    try:
        from scanner.awakening_detector import start_awakening_monitor
        start_awakening_monitor(app, interval_sec=30)
        print("  Awakening Mon  : ✅ enabled (every 30s)")
    except Exception as e:
        log.warning("awakening_monitor_init_err", err=str(e))
        print(f"  Awakening Mon  : ❌ ({type(e).__name__})")
...
def main():
    ...
    from ui.handlers import (
        ...
        cmd_watch, cmd_awaken, cmd_awaken_now, cmd_awaken_threshold,
    ...
    # Tier B: Early warning / awakening detection
    app.add_handler(CommandHandler("watch", cmd_watch))
    app.add_handler(CommandHandler("awaken", cmd_awaken))
    app.add_handler(CommandHandler("awaken_now", cmd_awaken_now))
    app.add_handler(CommandHandler("awaken_threshold", cmd_awaken_threshold))
    app.add_handler(CommandHandler("awaken_off", cmd_awaken_off))
    app.add_handler(CommandHandler("awaken_on", cmd_awaken_on))
    # Tier B: Reversal detection (trend reversal warnings)
    app.add_handler(CommandHandler("reversal", cmd_reversal))
    app.add_handler(CommandHandler("reversal_now", cmd_reversal_now))
    app.add_handler(CommandHandler("reversal_threshold", cmd_reversal_threshold))
    ...
    # Start the awakening system (watch list refresh + awakening scan)
    from scanner.orchestrator import start_awakening_system
    start_awakening_system(app)
