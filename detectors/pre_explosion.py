...
async def detect_pre_explosion(snap: MarketSnapshot) -> SourceVerdict:
    ...
    # Guard D: RSI overbought on daily — never call it "accumulation" if RSI 1d > 70
    df_1d = snap.klines_1d
    if df_1d is not None and len(df_1d) >= 15:
        try:
            from scorer.trend_maturity import calculate_rsi
            rsi_1d = calculate_rsi(df_1d["c"].astype(float), 14)
            rsi_6_1d = calculate_rsi(df_1d["c"].astype(float), 6)
            if rsi_1d > 72 or rsi_6_1d > 85:
                v.score = 25
                v.confidence = 0.75
                v.reasons = [
                    f"❌ RSI 1d مرتفع ({rsi_1d:.0f}/{rsi_6_1d:.0f}) — "
                    f"السعر في ذروة شراء، ليس تراكم"
                ]
                return v
        except Exception:
            pass

    # Guard E: Price stretched too far above EMA21 on 4h
    df_4h = snap.klines_4h
    if df_4h is not None and len(df_4h) >= 25:
        try:
            from scorer.trend_maturity import calculate_price_ema_distance_pct
            dist_pct = calculate_price_ema_distance_pct(df_4h, 21)
            if dist_pct > 15:
                v.score = 28
                v.confidence = 0.72
                v.reasons = [
                    f"❌ السعر متمدد +{dist_pct:.1f}% فوق EMA21(4h) — "
                    f"الترند ناضج جداً، ليس تراكم"
                ]
                return v
        except Exception:
            pass

    # Guard F: Bollinger upper band breakout on 1h (overextended)
    df_1h = snap.klines_1h
    if df_1h is not None and len(df_1h) >= 25:
        try:
            from scorer.trend_maturity import calculate_bollinger
            bb = calculate_bollinger(df_1h["c"].astype(float), 20, 2.0)
            if bb["upper"] > 0 and snap.price > bb["upper"] * 1.02:
                v.score = 30
                v.confidence = 0.7
                v.reasons = [
                    f"❌ السعر +{((snap.price/bb['upper']-1)*100):.1f}% فوق Bollinger upper — "
                    f"تمدد، ليس تراكم"
                ]
                return v
        except Exception:
