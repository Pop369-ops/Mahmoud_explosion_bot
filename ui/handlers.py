...
# ───────────────────────────────────────────────────────────────
# REVERSAL DETECTION COMMANDS (Tier B reversal alerts)
# ───────────────────────────────────────────────────────────────
async def cmd_reversal(u: Update, c: ContextTypes.DEFAULT_TYPE):
    """Show recent reversal alerts (last 4 hours)."""
    from scanner.reversal_detector import (
        get_recent_reversal_alerts, format_reversal_history,
    )
    try:
        recent = get_recent_reversal_alerts(hours=4)
        msg = format_reversal_history(recent, hours=4)
        await u.message.reply_text(msg, parse_mode="Markdown")
    except Exception as e:
        await u.message.reply_text(f"❌ {e}")


async def cmd_reversal_now(u: Update, c: ContextTypes.DEFAULT_TYPE):
    """Force-run reversal scan immediately on trending coins."""
    ...
    msg = await u.message.reply_text(
        f"🔄 جاري فحص العملات النشطة للانعكاسات (threshold {threshold})..."
    )
    try:
        alerts = await scan_for_reversals(
            chat_id=chat_id, bot=c.bot, threshold=threshold
        )
        if not alerts:
            await msg.edit_text(
                "😴 *لا انعكاس مكتشف الآن*\n\n"
                "_threshold: 60+, تأكيدات: 2+_",
                parse_mode="Markdown",
            )
        else:
            await msg.edit_text(
                f"⚡ تم إرسال `{len(alerts)}` تنبيه انعكاس.",
                parse_mode="Markdown",
            )
    except Exception as e:
        await msg.edit_text(f"❌ {e}")
...
async def cmd_reversal_threshold(u: Update, c: ContextTypes.DEFAULT_TYPE):
    """Set reversal threshold: /reversal_threshold 70"""
    chat_id = u.effective_chat.id
    if not c.args:
        cfg = state.get_user_cfg(chat_id)
        await u.message.reply_text(
            f"الحد الحالي: `{cfg.get('reversal_threshold', 60)}/100`\n"
            f"للتعديل: /reversal_threshold 70\n"
            f"النطاق المنصوح به: 55-80",
            parse_mode="Markdown",
        )
        return
    try:
        ...
        if val < 30 or val > 100:
            await u.message.reply_text("❌ النطاق المسموح: 30-100")
            return
        state.update_user_cfg(chat_id, reversal_threshold=val)
        await u.message.reply_text(
            f"✅ تم تعيين الحد إلى `{val}/100`",
            parse_mode="Markdown",
        )
    except (ValueError, IndexError):
        await u.message.reply_text("❌ الاستخدام: /reversal_threshold 70")


async def cmd_reversal_off(u: Update, c: ContextTypes.DEFAULT_TYPE):
    """Disable reversal alerts."""
    state.update_user_cfg(u.effective_chat.id, reversal_enabled=False)
    await u.message.reply_text(
        "🔕 *تم إيقاف تنبيهات الانعكاس*\n\n"
        "للتفعيل: /reversal\\_on",
        parse_mode="Markdown",
    )


async def cmd_reversal_on(u: Update, c: ContextTypes.DEFAULT_TYPE):
    """Enable reversal alerts."""
    state.update_user_cfg(u.effective_chat.id, reversal_enabled=True)
    await u.message.reply_text(
        "🔔 *تم تفعيل تنبيهات الانعكاس*\n\n"
        "سيتم فحص العملات النشطة كل دقيقة لاكتشاف الانعكاسات.\n"
        "للإيقاف: /reversal\\_off",
        parse_mode="Markdown",
    )


async def cmd_check_reversal(u: Update, c: ContextTypes.DEFAULT_TYPE):
    """Manually check a specific symbol for reversal: /check_reversal BTCUSDT"""
    if not c.args:
        await u.message.reply_text(
            "❌ الاستخدام: /check_reversal SYMBOL\nمثال: /check_reversal BTCUSDT"
        )
        return

    symbol = c.args[0].upper()
    if not symbol.endswith("USDT"):
        symbol += "USDT"

    msg = await u.message.reply_text(f"🔄 جاري فحص {symbol} للانعكاس...")
    try:
        from data_sources.binance import binance
        from scorer.reversal_engine import analyze_reversal, render_reversal_summary
        ...
        if df_1h is None or len(df_1h) < 50:
            await msg.edit_text(f"❌ بيانات غير كافية لـ {symbol}")
            return
        ...
        snap = _Snap()
        snap.symbol = symbol
        snap.price = float(df_1h["c"].iloc[-1])
        snap.klines = df_1h
        ...
        m = f"🔍 *تحليل {symbol}*\n"
        m += f"━━━━━━━━━━━━━━━━━━━━━\n\n"
        m += f"💰 *السعر:* `${snap.price:.6g}`\n\n"

        m += f"📊 *Trend Maturity:*\n"
        m += f"  • RSI 1h: `{maturity.rsi_1h:.0f}` | RSI 4h: `{maturity.rsi_4h:.0f}` | RSI 1d: `{maturity.rsi_1d:.0f}`\n"
        m += f"  • EMA21 distance: 1h `{maturity.ema_distance_1h_pct:+.1f}%`, 4h `{maturity.ema_distance_4h_pct:+.1f}%`\n"
        m += f"  • شموع صاعدة متتالية: 1h:`{maturity.consecutive_green_1h}`, 4h:`{maturity.consecutive_green_4h}`, 1d:`{maturity.consecutive_green_1d}`\n"

        if maturity.is_overextended_long:
            m += f"  ⚠️ *متمدد للأعلى:* {'؛ '.join(maturity.reasons[:2])}\n"
        elif maturity.is_overextended_short:
            m += f"  ⚠️ *متمدد للأسفل*\n"
        else:
            m += f"  ✅ ليس متمدداً\n"
        ...
        if report.direction == "neutral":
            ...
            m += f"  • الاتجاه: `{report.direction}`\n"
            m += f"  • القوة: `{report.total_strength}/100`\n"
            m += f"  • التأكيدات: `{report.confirms_count}`\n\n"
