...
def build_awakening_alert(awake) -> str:
    """Tier B early warning — fired by liquidity buildup before price moves.
    awake = AwakeningResult from detectors.liquidity_buildup
    """
    if awake.direction == "long":
        icon = "🟢"
        title = "إيقاظ سيولة صاعد"
        action = "🐂 محتمل ارتفاع"
    elif awake.direction == "short":
        icon = "🔴"
        title = "إيقاظ سيولة هابط"
        action = "🐻 محتمل هبوط"
    else:
        return ""

    bar_full = int(awake.total_score / 10)
    bar = "█" * bar_full + "░" * (10 - bar_full)

    m = f"⚠️ *تنبيه مبكر — {title}* {icon}\n"
    m += f"⏰ {now_riyadh_str()}\n"
    m += f"━━━━━━━━━━━━━━━━━━━\n"
    m += f"💎 *العملة:* `{awake.symbol}`\n"
    m += f"💰 *السعر الحالي:* `${awake.price:.6g}`\n"
    m += f"🎯 *الاتجاه المحتمل:* {action}\n"
    m += f"━━━━━━━━━━━━━━━━━━━\n"
    m += f"📊 *قوة الإيقاظ:* `{awake.total_score}/100`\n"
    m += f"`{bar}`\n"
    m += f"✅ *إشارات نشطة:* `{awake.signals_fired}/6`\n"
    m += f"━━━━━━━━━━━━━━━━━━━\n\n"

    if awake.reasons_ar:
        m += f"📡 *الإشارات المرصودة:*\n"
        for r in awake.reasons_ar[:6]:
            m += f"• {r}\n"
        m += "\n"

    m += f"━━━━━━━━━━━━━━━━━━━\n"
    m += f"💡 *ملاحظة:*\n"
    m += f"هذا تنبيه *مبكر* — السعر لم ينفجر بعد.\n"
    m += f"البوت يرصد تجمع سيولة قبل الحركة.\n\n"
    m += f"📌 *التوصية:*\n"
    m += f"• راقب الشارت يدوياً (1m / 5m)\n"
    m += f"• انتظر تأكيد إضافي قبل الدخول\n"
    m += f"• 30-50% من هذه التنبيهات لا تتحقق\n"
    m += f"• إن دخلت — استخدم size صغير\n\n"
    m += f"🔔 إن تأكد الانفجار، ستصلك إشارة مع SL/TP\n"
    m += f"_تنبيه تعليمي — ليس توصية مالية_"
    return m
